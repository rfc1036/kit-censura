...
# ---------------------------
TARGET_SUBSTR = "elenco_siti_inibiti_giochi.txt"
...
# Download helper
# ---------------------------
def guess_filename(file_url, headers=None, default="elenco_siti_inibiti_giochi.txt"):
    if headers:
...
def main():
    ap = argparse.ArgumentParser(
        description="Scarica 'Elenco dei siti soggetti ad inibizione - txt' (ADM), pulisce e stampa/salva l'elenco (LF, ordinato, no duplicati)."
    )
    ...
    if args.output is None:
        ...
        if out_arg.exists() and out_arg.is_dir():
            fname = ensure_txt_name(guess_filename(txt_url, headers=headers, default="elenco_siti_inibiti_giochi.txt"))
            dest = out_arg / fname
        else:
